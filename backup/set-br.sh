...
export izin="https://raw.githubusercontent.com/Zeadxt/kzl/main/ip"
#########################

BURIQ () {
    curl -sS ${izin} > /root/tmp
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS ${izin} | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
wget -q -O /root/.config/rclone/rclone.conf "https://raw.githubusercontent.com/Zeadxt/lpk/main/backup/rclone.conf"
...
wget -q -O /usr/bin/backup "https://raw.githubusercontent.com/Zeadxt/lpk/main/backup/backup.sh" && chmod +x /usr/bin/backup
wget -q -O /usr/bin/restore "https://raw.githubusercontent.com/Zeadxt/lpk/main/backup/restore.sh" && chmod +x /usr/bin/restore
wget -q -O /usr/bin/cleaner "https://raw.githubusercontent.com/Zeadxt/lpk/main/backup/logcleaner.sh" && chmod +x /usr/bin/cleaner
