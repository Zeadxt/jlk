...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS export izin="https://raw.githubusercontent.com/Zeadxt/kzl/main/ip" | grep $MYIP | awk '{print $2}')
...
LLatest=`date`
Get_Data () {
git clone https://github.com/Zeadxt/backup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    ...
    git remote add origin https://github.com/Zeadxt/backup
    git push -f https://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@github.com/Zeadxt/backup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/Zeadxt/backup/main/$NameUser/$NameUser.zip"
sleep 1
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}              • FRRE TUNNELING PROJECT •                $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo
