...
cd
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS ${izin} | grep $MYIP | awk '{print $2}')
cekdata=$(curl -sS https://raw.githubusercontent.com/Zeadxt/backup/main/$NameUser/$NameUser.zip | grep 404 | awk '{print $1}' | cut -d: -f1)
...
wget -q -O /root/backup/backup.zip "https://raw.githubusercontent.com/Zeadxt/backup/main/$NameUser/$NameUser.zip" &> /dev/null
...
echo -e "$COLOR1│${NC}  [ ${green}INFO${NC} ] • Done... Successfully - Script By FREE TUNNELING PROJECT"
sleep 1
rm -f /root/backup/backup.zip &> /dev/null
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
